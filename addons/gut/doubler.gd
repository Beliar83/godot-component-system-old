...
# Private
# ###############
func _write_file(target_path, dest_path, ignored_methods=null, override_path=null):
    var script_methods = _get_methods(target_path, ignored_methods)
...
func _double_scene_and_script(target_path, dest_path, ignored_methods=null):
    ...
    if(script_path):
        var double_path = _double(script_path, ignored_methods, target_path)
        var dq = '"'
        var f = File.new()
...
func _get_methods(target_path, ignored_methods=null):
    var obj = load(target_path).new()
    if ignored_methods == null:
    ...
    for i in range(methods.size()):
        ...
        if(methods[i]['flags'] == 65):
            if(!ignored_methods.has(methods[i]['name']) and !method_names.has(methods[i]['name'])):
...
func _double(obj, ignored_methods=null, override_path=null):
    var temp_path = _get_temp_path(obj)
    _write_file(obj, temp_path, ignored_methods, override_path)
    _double_count += 1
    return temp_path
...
func double_scene(path, ignored_methods=null):
    var temp_path = _get_temp_path(path)
    _double_scene_and_script(path, temp_path, ignored_methods)
    return load(temp_path)

func double(path, ignored_methods=null):
    return load(_double(path, ignored_methods))
